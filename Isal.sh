#!/bin/bash

echo  -n "Masukkan password Anda Di Sini :"
read pwd ;

if  [ $pwd = "arifisal" ]
then 
echo "\033[33;1m' "Password Anda Bener"
else 
echo "\033[32;1m' "Password Anda Salah"
echo "\033[35;1m' "Silahkan Masukkan Lagi"
fi

echo "green='\033[32;1m'  
echo "purple='\033[35;1m'
echo "cyan='\033[36;1m'
echo "red='\033[31;1m'
echo "white='\033[37;1m'                                           
echo "yellow='\033[33;1m'

echo "╔═\033[91m█████████████████████  ╦╔═╗ ╔╘╦═╗╔═≑╗
echo "\033[94m║ \033[91m█████████████████████  \033[90m║║ ╫ ║ ╫ ║║\033[95m◎◎\033[90m║
echo "\033[94m║ \033[97m█████████████████████  \033[97m╩╝ ╚═╝╼╩═╝╚══╝"
echo "\033[94m╠═\033[97m█████████████████████ ╔═╗ ╽╔═╗╔═╗╦╔≓═╗
echo "\033[94m║\033[95m─────[ \033[92mINDONESIA \033[95m]────\033[90m ║ ╫ ║╠╼ ╚═╗║╠══╣
echo "\033[94m║\033[95m────[ \033[93mDin-zUgex95\033[95m ]───\033[91m╾╝ ╚≒╝╚═╝╼═╝╩╯  ╿
echo "\033[94m╚═─══─══─══─══─══─══─═\033[95m• \033[93m────────────────
sleep1
echo "===================================================================================="
echo  "\033[36;1m' "Name = "faisal"
echo  "\033[33;1m' "Alamat = "cirebon"
echo  "\033[31;1m' "Team = "ngawur"
echo  "\033[37;1m' "No wa = "085xxxxxxxxx"
echo "===================================================================================="
sleep 2
echo

